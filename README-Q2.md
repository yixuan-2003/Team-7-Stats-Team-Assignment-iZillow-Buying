...
Feature engineering cuts CV error 44% versus the single-variable model of Q1. All rows
were re-run across ten random splits and vary by at most $540.

**Ablation** (drop one block from the full spec, see what breaks):

| Block removed | Δ RMSE |
|---|---|
| age block | **+$5,539** (10/10 splits) |
| slope dummies | **+$4,187** (10/10 splits) |
| bath/bed block | +$978 |
| basement block | +$819 |
| remodel info | +$477 |
| log size transforms | +$202 |
| land block | +$67 |
| neighborhood *intercept* dummies | −$388 (sd $289, sign flips in 1/10 — treat as "adds nothing") |
| style dummies | −$201 (negligible) |
...
3. **The `NB_*` intercept dummies add nothing** once `SLP_*` is in. Dropping them changes
   CV RMSE by about −$388 across ten splits (sd $289, sign flips in 1 of 10), so treat
   this as "redundant", not "removing them helps". Either drop them or let ridge shrink
   them (ridge beats OLS by ~$650 averaged over ten splits).
